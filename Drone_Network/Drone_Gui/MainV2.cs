    ...
    public partial class MainV2 : Form
    {
        ...
        */
        ...
        public MainV2()
        {
            ...
            {
                MissionPlanner.Utilities.ThemeManager.SetTheme((MissionPlanner.Utilities.ThemeManager.Themes)Enum.Parse(typeof(MissionPlanner.Utilities.ThemeManager.Themes), MainV2.config["theme"].ToString()));

                if (MissionPlanner.Utilities.ThemeManager.CurrentTheme == MissionPlanner.Utilities.ThemeManager.Themes.Custom)
                {
                    try
                    {
                        MissionPlanner.Utilities.ThemeManager.BGColor = Color.FromArgb(int.Parse(MainV2.config["theme_bg"].ToString()));
                        MissionPlanner.Utilities.ThemeManager.ControlBGColor = Color.FromArgb(int.Parse(MainV2.config["theme_ctlbg"].ToString()));
                        MissionPlanner.Utilities.ThemeManager.TextColor = Color.FromArgb(int.Parse(MainV2.config["theme_text"].ToString()));
                        MissionPlanner.Utilities.ThemeManager.ButBG = Color.FromArgb(int.Parse(MainV2.config["theme_butbg"].ToString()));
                        MissionPlanner.Utilities.ThemeManager.ButBorder = Color.FromArgb(int.Parse(MainV2.config["theme_butbord"].ToString()));
                    }
                    catch { log.Error("Bad Custom theme - reset to standard"); MissionPlanner.Utilities.ThemeManager.SetTheme(MissionPlanner.Utilities.ThemeManager.Themes.BurntKermit); }
                }

                if (MissionPlanner.Utilities.ThemeManager.CurrentTheme == MissionPlanner.Utilities.ThemeManager.Themes.HighContrast)
                {
        ...
        private void BGLoadAirports(object nothing)
        {
            ...
            try
            {
                MissionPlanner.Utilities.Airports.ReadOurairports(Application.StartupPath + Path.DirectorySeparatorChar + "airports.csv");
        ...
        private void ResetConnectionStats()
        {
            ...
            if (this.connectionStatsForm != null && connectionStatsForm.Visible)
            {
                ...
                MissionPlanner.Utilities.ThemeManager.ApplyThemeTo(this.connectionStatsForm);
            }
        }
        ...
        private void doConnect()
        {
            ...
            try
            {
                ...
                if (_connectionControl.CMB_serialport.Text == "AUTO")
                {
                    ...
                    _connectionControl.CMB_serialport.Text = CommsSerialScan.portinterface.PortName;
                    _connectionControl.CMB_baudrate.Text = CommsSerialScan.portinterface.BaudRate.ToString();
                }
                    ...
                    try
                    {
                        ...
                        foreach (Firmware.software item in softwares)
                        {
                            ...
                            if (fields1[0] == fields2[0])
                            {
                                ...
                                if (ver2 > ver1)
                                {
                                    MissionPlanner.Common.MessageShowAgain(Strings.NewFirmware, Strings.NewFirmwareA + item.name + Strings.Pleaseup);
                                    break;
                                }
                ...
                try
                {
                    ...
                    if (speechEnable && speechEngine != null && (DateTime.Now - speechcustomtime).TotalSeconds > 30 && (MainV2.comPort.logreadmode || comPort.BaseStream.IsOpen))
                    {
                        if (MainV2.speechEngine.State == SynthesizerState.Ready)
                        {
                            ...
                            {
                                MainV2.speechEngine.SpeakAsync(MissionPlanner.Common.speechConversion(MainV2.getConfig("speechcustom")));
                            }
                    ...
                    if (speechEnable && speechEngine != null && (DateTime.Now - speechbatterytime).TotalSeconds > 30 && (MainV2.comPort.logreadmode || comPort.BaseStream.IsOpen))
                    {
                        ...
                        if (MainV2.getConfig("speechbatteryenabled") == "True" && MainV2.comPort.MAV.cs.battery_voltage <= warnvolt && MainV2.comPort.MAV.cs.battery_voltage >= 5.0)
                        {
                            if (MainV2.speechEngine.State == SynthesizerState.Ready)
                            {
                                MainV2.speechEngine.SpeakAsync(MissionPlanner.Common.speechConversion(MainV2.getConfig("speechbattery")));
                    ...
                    if (speechEnable && speechEngine != null && (DateTime.Now - speechlowspeedtime).TotalSeconds > 10 && (MainV2.comPort.logreadmode || comPort.BaseStream.IsOpen))
                    {
                        if (MainV2.getConfig("speechlowspeedenabled") == "True" && MainV2.comPort.MAV.cs.armed)
                        {
                            ...
                            if (MainV2.comPort.MAV.cs.airspeed < warnairspeed)
                            {
                                if (MainV2.speechEngine.State == SynthesizerState.Ready)
                                {
                                    MainV2.speechEngine.SpeakAsync(MissionPlanner.Common.speechConversion(MainV2.getConfig("speechlowairspeed")));
                            ...
                            else if (MainV2.comPort.MAV.cs.groundspeed < warngroundspeed)
                            {
                                if (MainV2.speechEngine.State == SynthesizerState.Ready)
                                {
                                    MainV2.speechEngine.SpeakAsync(MissionPlanner.Common.speechConversion(MainV2.getConfig("speechlowgroundspeed")));
                                    speechlowspeedtime = DateTime.Now;
                                }
                            }
                            else
                    ...
                    if (speechEnable && speechEngine != null && (MainV2.comPort.logreadmode || comPort.BaseStream.IsOpen))
                    {
                        ...
                        try
                        {
                            ...
                            if (MainV2.getConfig("speechaltenabled") == "True" && MainV2.comPort.MAV.cs.alt != 0.00 && (MainV2.comPort.MAV.cs.alt <= warnalt) && MainV2.comPort.MAV.cs.armed)
                            {
                                ...
                                {
                                    if (MainV2.speechEngine.State == SynthesizerState.Ready)
                                        MainV2.speechEngine.SpeakAsync(MissionPlanner.Common.speechConversion(MainV2.getConfig("speechalt")));
                                }
                            }
                        }
                        catch { } // silent fail
                    ...
                    if (armedstatus != MainV2.comPort.MAV.cs.armed && comPort.BaseStream.IsOpen)
                    {
                        ...
                        if (speechEnable && speechEngine != null)
                        {
                            ...
                            {
                                if (armedstatus)
                                    MainV2.speechEngine.SpeakAsync(MissionPlanner.Common.speechConversion(MainV2.getConfig("speecharm")));
                                else
                                    MainV2.speechEngine.SpeakAsync(MissionPlanner.Common.speechConversion(MainV2.getConfig("speechdisarm")));
                            }
                        }
                    }
        ...
        protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
        {
            ...
            {
                //Form frm = new temp();
                //MissionPlanner.Utilities.ThemeManager.ApplyThemeTo(frm);
                //frm.Show();
                return true;
            }
        ...
        public void ChangeUnits()
        {
            try
            {
                ...
                {
                    switch ((MissionPlanner.Common.distances)Enum.Parse(typeof(MissionPlanner.Common.distances), MainV2.config["distunits"].ToString()))
                    {
                ...
                {
                    switch ((MissionPlanner.Common.speeds)Enum.Parse(typeof(MissionPlanner.Common.speeds), MainV2.config["speedunits"].ToString()))
                    {
                        case MissionPlanner.Common.speeds.ms:
                            CurrentState.multiplierspeed = 1;
                            CurrentState.SpeedUnit = "m/s";
                            break;
                        case MissionPlanner.Common.speeds.fps:
                            CurrentState.multiplierdist = 3.2808399f;
                            CurrentState.SpeedUnit = "fps";
                            break;
                        case MissionPlanner.Common.speeds.kph:
                            CurrentState.multiplierspeed = 3.6f;
                            CurrentState.SpeedUnit = "kph";
                            break;
                        case MissionPlanner.Common.speeds.mph:
                            CurrentState.multiplierspeed = 2.23693629f;
                            CurrentState.SpeedUnit = "mph";
                            break;
                        case MissionPlanner.Common.speeds.knots:
        ...
        private void MainMenu_ItemClicked(object sender, ToolStripItemClickedEventArgs e)
        {
            foreach (ToolStripItem item in MainMenu.Items)
            {
                ...
                {
                    item.BackColor = MissionPlanner.Utilities.ThemeManager.ControlBGColor;
                }
                else
                {
