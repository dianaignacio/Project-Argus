...
namespace MissionPlanner
{
    public class MagCalib
    {
        ...
        static void prd_DoWork(object sender, ProgressWorkerEventArgs e, object passdata = null)
        {
            ...
            MissionPlanner.HIL.Vector3 centre = new MissionPlanner.HIL.Vector3();
            ...
            while (true)
            {
                ...
                //new HIL.MissionPlanner.HIL.Vector3((float)-((maxx + minx) / 2), (float)-((maxy + miny) / 2), (float)-((maxz + minz) / 2));
                ...
                if (datacompass1.Count > 100 && lastlsq.Second != DateTime.Now.Second)
                {
                    ...
                    lock (datacompass1)
                    {
                        ...
                        {
                            centre = new MissionPlanner.HIL.Vector3(lsq[0], lsq[1], lsq[2]);
                ...
                if (datacompass2.Count > 100 && lastlsq2.Second != DateTime.Now.Second)
                {
                    ...
                    lock (datacompass2)
                    {
                        ...
                        if (Math.Abs(lsq[0]) < 999)
                        {
                            MissionPlanner.HIL.Vector3 centre2 = new MissionPlanner.HIL.Vector3(lsq[0], lsq[1], lsq[2]);
                ...
                MissionPlanner.HIL.Vector3 point;

                point = new MissionPlanner.HIL.Vector3(rawmx, rawmy, rawmz) + centre;

                //find the mean radius                    
                float radius = 0;
                for (int i = 0; i < datacompass1.Count; i++)
                {
                    point = new MissionPlanner.HIL.Vector3(datacompass1[i].Item1, datacompass1[i].Item2, datacompass1[i].Item3);
                ...
                for (int j = 0; j <= factor; j++)
                {
                    ...
                    for (int i = 0; i <= factor2; i++)
                    {
                        ...
                        MissionPlanner.HIL.Vector3 point_sphere = new MissionPlanner.HIL.Vector3(
                        ...
                        {
                            point = new MissionPlanner.HIL.Vector3(datacompass1[k].Item1, datacompass1[k].Item2, datacompass1[k].Item3);
                        ...
                        // draw them all
                        //((ProgressReporterSphere)sender).sphere1.AimFor(new OpenTK.MissionPlanner.HIL.Vector3((float)point_sphere.x, (float)point_sphere.y, (float)point_sphere.z));
                        if (!found)
                        {
        ...
        {
            var offsets = new MissionPlanner.HIL.Vector3(p1[0], p1[1], p1[2]);
            var diagonals = new MissionPlanner.HIL.Vector3(1.0, 1.0, 1.0);
            var offdiagonals = new MissionPlanner.HIL.Vector3(0.0, 0.0, 0.0);
            if (p1.Length >= 6)
                diagonals = new MissionPlanner.HIL.Vector3(p1[3], p1[4], p1[5]);
            if (p1.Length >= 8)
                offdiagonals = new MissionPlanner.HIL.Vector3(p1[6], p1[7], p1[8]);
            ...
            {
                var mag = new MissionPlanner.HIL.Vector3(d.Item1, d.Item2, d.Item3);
        ...
        static double radius(MissionPlanner.HIL.Vector3 mag, MissionPlanner.HIL.Vector3 offsets, MissionPlanner.HIL.Vector3 diagonals, MissionPlanner.HIL.Vector3 offdiagonals)
        {
            //'''return radius give data point and offsets'''
            MissionPlanner.HIL.Vector3 mag2 = mag + offsets;
            var rot = new MissionPlanner.HIL.Matrix3(new MissionPlanner.HIL.Vector3(diagonals.x, offdiagonals.x, offdiagonals.y),
                           new MissionPlanner.HIL.Vector3(offdiagonals.x, diagonals.y, offdiagonals.z),
                           new MissionPlanner.HIL.Vector3(offdiagonals.y, offdiagonals.z, diagonals.z));
            mag2 = rot * mag2;
            return mag2.length();
        }
